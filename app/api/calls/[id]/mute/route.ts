...
export async function POST(
  ...
  try {
    ...
    const success = await vonageService.muteCall(call.vonageCallId);

    if (success) {
      return NextResponse.json({ message: 'Call muted successfully' });
    } else {
      return NextResponse.json({ error: 'Failed to mute call' }, { status: 500 });
    }
